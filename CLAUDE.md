...
## Estado actual del proyecto (2026-05-13 — sesión 13)
...
### Implementado en sesión 10 (2026-05-05)
- **Migración `20260429000003_add_semantic_cache` aplicada:** tabla `ai_response_cache` + índice HNSW halfvec(3072) ya existe en la BD. Docs pre-existentes corregidos: `UPDATE documents SET processing_status = 'done' WHERE is_active = true`.
- **`GET /api/auth/me`:** endpoint protegido por JWT. `GetMeController` usa `PrismaAuthRepository.findById()` (nuevo método). Devuelve `{ user: { id, email, role, profilePhotoUrl, mustChangePassword, createdAt } }`. Frontend: `authContext` llama `/api/auth/me` en mount en vez de leer localStorage ciegamente → datos siempre frescos del servidor. `isInitialized` state evita flash de redirect a `/login` mientras dura la llamada. `ProtectedRoute` retorna `null` hasta que `isInitialized` sea `true`.
- **correlationId con AsyncLocalStorage:** `backend/src/shared/context/requestContext.ts` — `AsyncLocalStorage<{ correlationId }>` + `getCorrelationId()`. Hook `onRequest` en `server.ts`: propaga el `reqId` de Fastify al header `x-request-id` de respuesta Y lo almacena en el ALS para toda la cadena async del request. `LLMCallbackHandler` incluye `[correlationId]` en cada log `START`, `END`, `ERROR` para trazabilidad completa.
...
### Implementado en sesión 13 (2026-05-13) — Dashboard IA para ADMIN
- **Migración `20260513000001_add_dashboard_metrics`:**
  - `ai_queries` ampliada: `provider VARCHAR(50) DEFAULT 'gemini'`, `input_tokens`, `output_tokens`, `input_tokens_estimated BOOLEAN`, `latency_ms`, índice `idx_ai_queries_provider_created (provider, created_at)`
  - Nueva tabla `ai_cache_stats (id, date, provider, model, hits, misses, created_at, updated_at)` — UNIQUE `(date, provider, model)`, índice en `date`
  - Nueva tabla `ai_job_history (id, document_id FK→documents CASCADE, provider, started_at, completed_at, duration_ms, status, error_message, attempt_number, created_at)` — índices en `document_id` y `(status, created_at)`
- **`LLMCallbackHandler`:** `lastCallMetrics` público (`inputTokens, outputTokens, totalTokens, latencyMs, inputTokensEstimated`). Tokens de prompt estimados como `chars/4` cuando Gemini streaming retorna `promptTokens: 0` (confirmado comportamiento real), marcados con `inputTokensEstimated: true`.
- **`GeminiAIService`:** `handler` como propiedad de clase (no anónimo en constructor). Expone `getLastCallMetrics()`.
- **`AIService` interface:** añadido `LLMCallMetrics` type + `getLastCallMetrics?()` opcional para no romper otras implementaciones.
- **`AskAIQuestionUseCase`:** después de cada llamada LLM persiste métricas reales en `ai_queries` (provider, tokens, latencia). Hace upsert en `ai_cache_stats` en cada HIT (hits+1) y MISS (misses+1) — ambas operaciones async sin bloquear la respuesta.
- **`DocumentWorker`:** inserta en `ai_job_history` al completar cada job: `status: 'done'` con `duration_ms`, o `status: 'failed'` con `error_message` y `attempt_number`. Async para no afectar el flujo BullMQ.
- **`AdminDashboardController`** (`interfaces/controllers/admin/`): 4 endpoints GET, todos protegidos con `PERMISSIONS.VIEW_ADMIN_DASHBOARD` (solo ADMIN):
  - `GET /api/admin/dashboard/overview` — tarjetas resumen: queries/tokens (7d + allTime), cache hit rate, usuarios activos, jobs completados/fallidos, duración promedio
  - `GET /api/admin/dashboard/llm-usage` — breakdown diario por proveedor/modelo (últimos 7 días): queries, input/output/total tokens, latencia promedio
  - `GET /api/admin/dashboard/cache` — dailyStats + overallHitRate + totales (últimos 7 días)
  - `GET /api/admin/dashboard/queue` — dailyStats (done/failed/avgDuration) + `recentFailures` (últimas 20 fallas con documentName y errorMessage)
- **Frontend — `AdminDashboardPage`** (`pages/AdminDashboardPage.tsx`): 6 tarjetas resumen + 6 gráficas Recharts (consultas/día, tokens/día, latencia/día, hits-vs-misses, jobs/día, duración media) + tabla de jobs fallidos recientes. Usa `dashboardService.ts` en `features/admin/`.
- **`Header.tsx`:** ícono `LayoutDashboard` de lucide-react visible solo para ADMIN (a la izquierda del avatar). Navega a `/admin/dashboard`.
- **`App.tsx`:** ruta `/admin/dashboard` protegida con `allowedRoles={['ADMIN']}`.
- **`PERMISSIONS`:** añadido `VIEW_ADMIN_DASHBOARD: ['ADMIN']`.

### Migraciones existentes (completas)
1. `20260312191154_init_rag_schema`
2. `20260324000000_update_embedding_dimensions`
3. `add_phone_to_users`
4. `profile_image_text`
5. `20260424164028_add_password_reset_tokens`
6. `20260424183351_add_password_change_tracking`
7. `20260427172226_add_refresh_tokens`
8. `20260429000001_add_hnsw_and_cleanup_indexes`
9. `20260429000002_add_document_processing_status`
10. `20260429000003_add_semantic_cache`
11. `20260513000001_add_dashboard_metrics`

### Schema DB — modelos actuales (16)
`users, roles, user_roles, conversations, messages, documents, document_chunks, document_embeddings, ai_queries, audit_logs, models, llm_models, password_reset_tokens, refresh_tokens, ai_response_cache, ai_cache_stats, ai_job_history`

### Deuda técnica pendiente
- NINGUNA. Proyecto terminado con tests (sesión 11) y dashboard IA (sesión 13).
...
/admin/dashboard    → ProtectedRoute allowedRoles=[ADMIN] > DashboardLayout > AdminDashboardPage
...
10. `20260429000003_add_semantic_cache` — Tabla `ai_response_cache` + índice HNSW halfvec(3072)
11. `20260513000001_add_dashboard_metrics` — `ai_queries` ampliada (provider, tokens desglosados, latencia) + tablas `ai_cache_stats` y `ai_job_history`
...
15. ~~**Endpoint `GET /api/auth/me`**~~ ✅ Implementado sesión 10 (GetMeController, findById en PrismaAuthRepository, frontend usa /me en mount)
16. ~~**Logging con correlationId**~~ ✅ Implementado sesión 10 (AsyncLocalStorage, hook onRequest, LLMCallbackHandler)
17. ~~**Tests Jest + Supertest**~~ ✅ Implementado sesión 11 (74 tests, 6 suites, 100% green)
18. ~~**Dashboard IA para ADMIN**~~ ✅ Implementado sesión 13 (métricas LLM, caché, queue; Recharts; 4 endpoints)
...
- ~~**Logging estructurado de errores:**~~ ✅ Implementado sesión 10 (AsyncLocalStorage, x-request-id header, LLMCallbackHandler con correlationId).
- ~~**Endpoint `/me`**~~ ✅ Implementado sesión 10.
- ~~**Tests unitarios/integración**~~ ✅ Implementado sesión 11 (74 tests, Jest + Supertest).
- ~~**Dashboard de métricas IA**~~ ✅ Implementado sesión 13 (LLM usage, caché hit/miss, BullMQ job history).
